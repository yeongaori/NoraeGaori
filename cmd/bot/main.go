...
func main() {
	// Tune GC to reduce audio stuttering caused by GC pauses.
	// Default GOGC=100 triggers a GC cycle every time heap doubles, which at high
	// allocation rates (50 Opus packets/sec × N guilds) can pause goroutines for
	// 10-100ms — audible as stutters. GOGC=300 triples the heap-growth threshold,
	// reducing GC frequency to ~1/3 of default at the cost of slightly more memory.
	// The 200-frame PCM buffer (~4s) absorbs the longer inter-GC interval.
