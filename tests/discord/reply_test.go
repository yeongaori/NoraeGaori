...
func replyInteraction(isMessageCommand bool) *discordgo.InteractionCreate {
	if isMessageCommand {
		return &discordgo.InteractionCreate{Interaction: &discordgo.Interaction{GuildID: replyGuildID, ChannelID: replyChannelID, Token: "message_reply_" + replyChannelID}}
	}
	return discordtest.WithToken(&discordgo.InteractionCreate{Interaction: &discordgo.Interaction{Type: discordgo.InteractionApplicationCommand, GuildID: replyGuildID}})
}

func newResponder(session *discordgo.Session, message *discordgo.Message) *discord.MessageResponse {
	return &discord.MessageResponse{Session: session, ChannelID: replyChannelID, Message: message, OriginalMsgID: "origin"}
}
...
func TestReplyHelpersOnEveryPath(t *testing.T) {
	respondError := func(s *discordgo.Session, i *discordgo.InteractionCreate) { discord.RespondError(s, i, "broken") }
	respondSuccess := func(s *discordgo.Session, i *discordgo.InteractionCreate) { discord.RespondSuccess(s, i, "done") }
	respondEmbed := func(s *discordgo.Session, i *discordgo.InteractionCreate) {
		discord.RespondEmbed(s, i, &discordgo.MessageEmbed{Title: "answer"})
	}
	deferResponse := func(s *discordgo.Session, i *discordgo.InteractionCreate) { discord.DeferResponse(s, i) }
	followUpMessage := func(s *discordgo.Session, i *discordgo.InteractionCreate) { discord.FollowUpMessage(s, i, "later") }
	followUpEmbed := func(s *discordgo.Session, i *discordgo.InteractionCreate) {
		discord.FollowUpEmbed(s, i, &discordgo.MessageEmbed{Title: "summary"})
	}
	loadingTitle := messages.T(replyGuildID).Titles.Loading
...
func TestUpdateResponseEmbedOnEveryPath(t *testing.T) {
	...
	t.Run("a slash command", func(t *testing.T) {
		...
		if err := discord.UpdateResponseEmbed(session, replyInteraction(false), embed); err != nil {
	...
	t.Run("a text command with a reply", func(t *testing.T) {
		...
		defer discord.RegisterResponder(ic.Token, newResponder(session, &discordgo.Message{ID: storedMessageID}))()
...
func TestUpdateResponseEmbedWithComponentsOnEveryPath(t *testing.T) {
	...
	t.Run("a slash command", func(t *testing.T) {
		...
		if err := discord.UpdateResponseEmbedWithComponents(session, replyInteraction(false), embed, components); err != nil {
...
func TestGetResponseMessageOnEveryPath(t *testing.T) {
	t.Run("a slash command", func(t *testing.T) {
		...
		message, err := discord.GetResponseMessage(session, replyInteraction(false))
	...
	t.Run("a text command with a reply", func(t *testing.T) {
		...
		defer discord.RegisterResponder(ic.Token, newResponder(session, stored))()

		if message, err := discord.GetResponseMessage(session, ic); err != nil || message != stored {
	...
	for name, hasResponder := range map[string]bool{"a text command before its reply": true, "a text command without responder": false} {
		t.Run(name, func(t *testing.T) {
			...
			if message, err := discord.GetResponseMessage(session, ic); err == nil || message != nil {
...
func TestCheckUserInBotVoiceChannel(t *testing.T) {
	...
	for name, check := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			channel, embed := discord.CheckUserInBotVoiceChannel(session, ic)
