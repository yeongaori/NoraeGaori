...
func TestTruncateToLimit(t *testing.T) {
	if got := discord.TruncateToLimit("hello", 10); got != "hello" {
		t.Errorf("short string changed: %q", got)
	}
	got := discord.TruncateToLimit("abcdefghij", 5)
	if len(got) > 5 {
	...
	tr := discord.TruncateToLimit(multibyte, 10)
	if len(tr) > 10 {
...
func TestSplitLinesIntoChunks(t *testing.T) {
	...
	chunks := discord.SplitLinesIntoChunks(lines, 8)
	if len(chunks) == 0 {
