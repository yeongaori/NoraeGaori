...
const memberGuildID = "member-guild"
...
func TestMessageMemberUsesTheMemberSentWithTheMessage(t *testing.T) {
	...
	member := discord.MessageMember(session, message)

	if member.User != message.Author || member.GuildID != memberGuildID {
...
func TestMessageMemberFetchesTheMemberWhenTheMessageHasNone(t *testing.T) {
	...
	member := discord.MessageMember(session, memberMessage(nil))

	if member.User == nil || member.User.ID != "fetched" || len(member.Roles) != 1 {
		t.Errorf("member = %+v, want the member returned by Discord", member)
	}
	sent := requests()
	if len(sent) != 1 || sent[0].Method != "GET" || sent[0].Path != "/guilds/"+memberGuildID+"/members/author" {
...
func TestCreatePseudoInteractionCarriesTheGivenMember(t *testing.T) {
	...
	interaction := discord.CreatePseudoInteraction(message, member, "play", nil, nil)
