...
func TestEveryoneRoleCanGrantAdministrator(t *testing.T) {
	...
	if !discord.IsGuildAdmin(session, guildID, memberOf("anyone")) {
		t.Error("a member was refused although @everyone holds Administrator")
	}
	if !discord.IsGuildAdmin(session, guildID, memberOf("anyone", "member-role", "deleted-role")) {
...
func TestGuildAdminFallsBackToDiscordForAnUncachedGuild(t *testing.T) {
	...
	for name, check := range map[string]struct {
		...
		if got := discord.IsGuildAdmin(session, guildID, check.member); got != check.wantAdmin {
...
func TestGuildAdminRefusesWhenTheGuildCannotBeFound(t *testing.T) {
	...
	if discord.IsGuildAdmin(session, "missing-guild", memberOf("owner")) {
		t.Error("a member of a guild Discord cannot find was treated as an admin")
	}
	if discord.IsGuildAdmin(session, "missing-guild", nil) || discord.IsGuildAdmin(session, "missing-guild", &discordgo.Member{}) {
