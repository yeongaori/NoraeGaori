...
func TestConcurrentSwapAndMove(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			if idx%2 == 0 {
				queue.SwapSongs(guildID, idx%5, (idx+2)%5)
			} else {
				queue.MoveSong(guildID, idx%5, (idx+3)%5)
			}
		}(i)
	}
	wg.Wait()
...
func TestConcurrentSettings(t *testing.T) {
	...
	for i := 0; i < 30; i++ {
		...
		go func(idx int) {
			...
			switch idx % 3 {
			case 0:
				queue.SetVolume(guildID, float64((idx%10)*10))
			case 1:
				queue.SetRepeatMode(guildID, idx%3)
			case 2:
				queue.SetSponsorBlock(guildID, idx%2 == 0)
			}
		}(i)
	}
	wg.Wait()
