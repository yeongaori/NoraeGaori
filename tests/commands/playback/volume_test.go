...
func TestVolume(t *testing.T) {
	...
	commandtest.Run(t, "volume", playback.HandleVolume, []commandtest.Case{
		{
...
func TestRegisterAddsThePlaybackCommands(t *testing.T) {
	...
	playback.Register(commandStrings)

	commandtest.WantRegistered(t, map[string]commandtest.Registration{
		"nowplaying": {Handler: playback.HandleNowPlaying},
		"volume":     {Handler: playback.HandleVolume},
		"repeat":     {SettingKey: "repeat"},
		"pause":      {Handler: playback.HandlePause},
		"resume":     {Handler: playback.HandleResume},
		"skip":       {Handler: playback.HandleSkip},
		"seek":       {Handler: playback.HandleSeek},
		"stop":       {Handler: playback.HandleStop},
	})
}
