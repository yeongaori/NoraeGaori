...
func TestEscapeLinkText(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := messages.EscapeLinkText(c.in)
			if got != c.want {
...
func TestFormatBoldMaskedLink(t *testing.T) {
	got := messages.FormatBoldMaskedLink(`[MV] foo`, `https://example.com/watch?v=abc`)
...
func TestFormatMaskedLinkPipes(t *testing.T) {
	got := messages.FormatMaskedLink(`Music Video || Frederic "oddloop"`, `https://example.com/x`)
...
func TestEscapeURLClosingParen(t *testing.T) {
	got := messages.EscapeURL(`https://example.com/watch?v=abc(def)`)
