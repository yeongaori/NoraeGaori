...
func newTestPool(workerCount int, checkFn func(string) (bool, bool, error)) *AvailabilityWorkerPool {
...
func testJobs(urls ...string) []AvailabilityJob {
	jobs := make([]AvailabilityJob, 0, len(urls))
	for i, url := range urls {
		jobs = append(jobs, AvailabilityJob{URL: url, Index: i})
	}
	return jobs
}
...
func TestRetryExhaustedReturnsError(t *testing.T) {
	...
	finished := make(chan []AvailabilityResult, 1)
	go func() {
...
func TestBackoffDelayIsExponentialAndCapped(t *testing.T) {
	pool := newAvailabilityWorkerPool(1)
...
func TestCloseWithRetriesInFlight(t *testing.T) {
	pool := newAvailabilityWorkerPool(4)
	...
	batchResults := make(chan AvailabilityResult, 8)
...
func TestCheckBatchImmediateInvokesCallbackOncePerJob(t *testing.T) {
	...
	pool.CheckBatchImmediate(jobs, func(result AvailabilityResult) {
		seen[result.Index]++
	})
