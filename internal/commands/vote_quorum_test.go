...
func TestRequiredVotesInChannelIgnoresBotsAndOtherChannels(t *testing.T) {
	...
	for _, testCase := range cases {
		...
		got, err := requiredVotesInChannel(session, "g", "voice", resolveWithFetch)
		if err != nil {
...
func TestRequiredVotesInChannelCountsMembersAttachedToVoiceStates(t *testing.T) {
	...
	for _, mode := range []memberResolution{resolveFromCache, resolveWithFetch} {
		got, err := requiredVotesInChannel(session, "g", "voice", mode)
		if err != nil {
			t.Fatalf("requiredVotesInChannel returned %v, want nil", err)
		}
		if got != 2 {
...
func TestRequiredVotesInChannelSkipsTheBotItself(t *testing.T) {
	...
	got, err := requiredVotesInChannel(session, "g", "voice", resolveFromCache)
	if err != nil {
		t.Fatalf("requiredVotesInChannel returned %v, want nil", err)
	}
	if got != 1 {
...
func TestRequiredVotesInChannelCountsUnresolvedMembersOnTheHotPath(t *testing.T) {
	...
	cached, err := requiredVotesInChannel(session, "g", "voice", resolveFromCache)
	if err != nil {
		t.Fatalf("requiredVotesInChannel returned %v, want nil", err)
	}
	if cached != 2 {
...
func TestRequiredVotesInChannelReportsAnUnknownGuild(t *testing.T) {
	...
	if _, err := requiredVotesInChannel(session, "missing", "voice", resolveFromCache); err == nil {
