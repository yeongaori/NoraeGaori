...
func checkPanelState(songs []*queue.Song, guildOverrides player.TransitionStyleOverrides, autoSelect bool) panelState {
	return panelState{
...
func checkRowsFor(songs []*queue.Song) []transitionRow {
	state := checkPanelState(songs, player.TransitionStyleOverrides{}, true)
	return hydrateTransitionRows("check-guild", state, state.pairs)
}

func checkRowsWithGuild(songs []*queue.Song, guildOverrides player.TransitionStyleOverrides) []transitionRow {
...
func checkPanelLimits(c *panelCheckCollector) {
	...
	for _, testCase := range cases {
		state := checkPanelState(testCase.songs, player.TransitionStyleOverrides{}, true)
...
func checkPanelIdentifiers(c *panelCheckCollector) {
	state := checkPanelState(checkSongs(4, "Track"), player.TransitionStyleOverrides{}, true)
	rows := hydrateTransitionRows("check-guild", state, state.pairs)
	if len(rows) < 2 {
	...
	for _, menu := range firstSelects {
		...
		if !player.ValidTransitionStyle(category, queue.AutoMixStyleAuto) || player.TransitionStyleValues(category) == nil {
			parseFailures++
			continue
		}
...
func checkPanelRows(c *panelCheckCollector) {
	...
	fiftyState := checkPanelState(checkSongs(50, "Track"), player.TransitionStyleOverrides{}, true)
	...
	rows = checkRowsWithGuild(songs, player.TransitionStyleOverrides{Filter: "lowpass_in", Effect: "reverb_out_end"})
	...
	offState := checkPanelState(songs, player.TransitionStyleOverrides{EQ: "quick_bass"}, false)
	...
	idleState := checkPanelState(songs, player.TransitionStyleOverrides{}, true)
	...
	autoOutro := player.AutoOutroStyles(outroRow.fromAnalysis)
	outroValid := true
	for _, category := range transitionCategories {
		if !player.ValidTransitionStyle(category, autoOutro[category]) {
			outroValid = false
		}
