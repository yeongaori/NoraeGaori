...
type transitionRow struct {
	transitionPair
	fromAnalysis  *player.TrackAnalysis
	toAnalysis    *player.TrackAnalysis
...
type panelState struct {
	...
	guildOverrides player.TransitionStyleOverrides
	autoSelect     bool
	crossfade      bool
	autoMixBeats   int
...
func hydrateTransitionRow(guildID string, state panelState, pair transitionPair) transitionRow {
	fromAnalysis := player.LookupAnalysisForDisplay(guildID, pair.fromSong, player.AnalysisSegmentTail)

	var toAnalysis *player.TrackAnalysis
	var recipe player.TransitionRecipe
	var effective, source map[string]string

	if pair.isOutro() {
		recipe, effective, source = player.ResolveOutroStyles(fromAnalysis, state.autoSelect,
			state.guildOverrides, songStyleOverrides(pair.fromSong))
	} else {
		toAnalysis = player.LookupAnalysisForDisplay(guildID, pair.toSong, player.AnalysisSegmentHead)
		recipe, effective, source = player.ResolveTransitionStyles(fromAnalysis, toAnalysis, state.autoSelect,
			state.guildOverrides, songStyleOverrides(pair.fromSong))
	}
	...
	crossfadeFrames, _ := player.TransitionCrossfadeFrames(state.autoSelect, state.autoMixBeats, state.crossfadeSec, fromAnalysis)
	if clamped, _ := player.ClampLoopStyle(recipe.Loop, periodSec, crossfadeFrames); clamped != recipe.Loop {
...
func queueStyleOverrides(q *queue.Queue) player.TransitionStyleOverrides {
	return player.TransitionStyleOverrides{
...
func songStyleOverrides(song *queue.Song) player.TransitionStyleOverrides {
	if song == nil {
		return player.TransitionStyleOverrides{}
	}
	return player.TransitionStyleOverrides{
...
func autoStylesFor(state panelState, row transitionRow) map[string]string {
	if !state.autoSelect {
		_, styles, _ := player.ResolveTransitionStyles(nil, nil, false,
			player.TransitionStyleOverrides{}, player.TransitionStyleOverrides{})
		return styles
	}
	if row.isOutro() {
		return player.AutoOutroStyles(row.fromAnalysis)
	}
	return player.AutoTransitionStyles(row.fromAnalysis, row.toAnalysis)
}
...
func describeTrack(guildID string, analysis *player.TrackAnalysis, analyzing bool) string {
	panel := messages.T(guildID).AutoMixPanel
	if analysis == nil {
		if analyzing {
	...
	bpm, key, camelot, hasKey := player.AnalysisSummary(analysis)
	if bpm <= 0 {
		if analyzing {
			return panel.Analyzing
		}
...
func describeSongLine(guildID string, index int, song *queue.Song, analysis *player.TrackAnalysis, analyzing bool) string {
	return fmt.Sprintf("**%d.** %s · %s\n\n",
		index+1,
		messages.EscapeMarkdown(truncateRunes(song.Title, 50)),
...
func createTransitionPanelComponents(guildID string, rows []transitionRow, page, totalPages int, token string) []discordgo.MessageComponent {
	...
	for _, row := range rows {
		...
		options = append(options, discordgo.SelectMenuOption{
			Label:       truncateRunes(transitionLabel(guildID, row.transitionPair), discordLabelLimit),
			Description: truncateRunes(describeRecipe(guildID, row, false), discordLabelLimit),
...
func createTransitionEditorEmbed(guildID string, state panelState, row transitionRow, errorMessage string) *discordgo.MessageEmbed {
	...
	if delta, distance, ok := player.TransitionCompatibility(row.fromAnalysis, row.toAnalysis); ok {
	...
	fields := []*discordgo.MessageEmbedField{
		{
			...
			Value: fmt.Sprintf("%s\n%s",
				messages.EscapeMarkdown(truncateRunes(row.fromSong.Title, 80)),
	...
	if row.isOutro() {
		...
		fields = append(fields, &discordgo.MessageEmbedField{
			...
			Value: fmt.Sprintf("%s\n%s",
				messages.EscapeMarkdown(truncateRunes(row.toSong.Title, 80)),
	...
	if errorMessage != "" {
		fields = append(fields, &discordgo.MessageEmbedField{
			...
			Value:  truncateRunes(errorMessage, 1024),
			Inline: false,
		})
	}
	...
	return &discordgo.MessageEmbed{
		Color:  messages.ColorInfo,
		Title:  truncateRunes(fmt.Sprintf(panel.EditorTitle, transitionLabel(guildID, row.transitionPair)), 256),
		Fields: fields,
	}
}
...
func createTransitionEditorComponents(guildID string, state panelState, row transitionRow, token string) []discordgo.MessageComponent {
	...
	for _, category := range transitionCategories {
		...
		options := []discordgo.SelectMenuOption{{
			Label:       truncateRunes(prefix+panel.AutoOption, discordLabelLimit),
			Description: truncateRunes(fmt.Sprintf(panel.AutoOptionDesc, styleLabel(guildID, category, autoStyles[category])), discordLabelLimit),
			Value:       queue.AutoMixStyleAuto,
			Default:     current == queue.AutoMixStyleAuto,
		}}

		for _, style := range player.TransitionStyleValues(category) {
			if style == queue.AutoMixStyleAuto {
				continue
			}
			options = append(options, discordgo.SelectMenuOption{
				Label:   truncateRunes(prefix+styleLabel(guildID, category, style), discordLabelLimit),
				Value:   style,
...
func overrideForCategory(overrides player.TransitionStyleOverrides, category string) string {
	switch category {
	case "volume":
...
func HandleAutoMixPanel(s *discordgo.Session, i *discordgo.InteractionCreate) error {
	...
	if !ok || len(state.pairs) == 0 {
		RespondEmbed(s, i, messages.CreateErrorEmbed(panel.EmptyTitle, panel.EmptyDesc))
		return nil
	}
...
func OpenAutoMixPanelFromComponent(s *discordgo.Session, ic *discordgo.InteractionCreate) {
	guildID := ic.GuildID
	panel := messages.T(guildID).AutoMixPanel
