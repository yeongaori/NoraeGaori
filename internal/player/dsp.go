...
func clampFrequency(freq float64) float64 {
	...
	maxFreq := dspSampleRate * biquadMaxFreqRatio
	if freq > maxFreq {
		return maxFreq
	}
	return freq
...
func (f *biquad) setLowpass(freq, q float64) {
	...
	w0 := 2 * math.Pi * freq / dspSampleRate
	cosW := math.Cos(w0)
...
func newDelayLine() *delayLine {
	size := int(delayMaxSeconds * dspSampleRate)
	return &delayLine{
		left:  make([]float64, size),
		right: make([]float64, size),
		delay: int(0.25 * dspSampleRate),
		dry:   1,
	}
}

func (d *delayLine) setDelaySeconds(seconds float64) {
	samples := int(seconds * dspSampleRate)
	if samples < 1 {
		samples = 1
	}
