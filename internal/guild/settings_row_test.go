...
func watchSettingsChanges(t *testing.T, guildID string) func() int {
	...
	var mu sync.Mutex
	count := 0
	OnSettingsChange(func(changed string) {
		if changed != guildID {
			return
		}
		mu.Lock()
...
func TestSaveSettingCreatesTheRowWithTheConfiguredDefaultVolume(t *testing.T) {
	...
	if err := SaveSetting("new-guild", "sponsorblock", 1); err != nil {
...
func TestSaveSettingKeepsAnExistingVolume(t *testing.T) {
	...
	if err := SaveSetting("volume-guild", "volume", 70); err != nil {
		t.Fatalf("failed to save the volume: %v", err)
	}
	if err := SaveSetting("volume-guild", "normalization", 1); err != nil {
...
func TestSaveSettingReportsFailuresWithoutNotifying(t *testing.T) {
	...
	if err := SaveSetting("failing-guild", "no_such_column", 1); err == nil {
	...
	if err := SaveSetting("failing-guild", "sponsorblock", 1); err == nil {
...
func TestClearingThePrefixAndLanguageStoresNothing(t *testing.T) {
	dbtest.Setup(t)
	t.Cleanup(func() { InvalidateCaches("clearing-guild") })

	if err := SetPrefix("clearing-guild", "?"); err != nil {
		t.Fatalf("SetPrefix failed: %v", err)
	}
	if err := SetPrefix("clearing-guild", ""); err != nil {
		t.Fatalf("clearing the prefix failed: %v", err)
	}
	if err := SetLanguage("clearing-guild", ""); err != nil {
		t.Fatalf("clearing the language failed: %v", err)
	}

	InvalidateCaches("clearing-guild")
	if prefix, err := GetPrefix("clearing-guild"); err != nil || prefix != "" {
		t.Errorf("GetPrefix = (%q, %v) after clearing, want empty", prefix, err)
	}
	if language, err := GetLanguage("clearing-guild"); err != nil || language != "" {
...
func TestPrefixAndLanguageReportSaveFailures(t *testing.T) {
	...
	if err := SetPrefix("closed-guild", "?"); err == nil {
		t.Error("SetPrefix on a closed database succeeded")
	}
	if err := SetLanguage("closed-guild", "ko"); err == nil {
...
func TestPrefixAndLanguageWritesNotifySettingsChanges(t *testing.T) {
	...
	if err := SetPrefix("notify-guild", "?"); err != nil {
		t.Fatalf("SetPrefix failed: %v", err)
	}
	if err := SetLanguage("notify-guild", "ko"); err != nil {
