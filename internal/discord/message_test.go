...
func TestParseCommandOptions(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			options := parseCommandOptions(tc.cmd.Options, tc.args)
...
func TestEdgeCases(t *testing.T) {
	t.Run("Nil options", func(t *testing.T) {
		if options := parseCommandOptions(nil, []string{"test"}); options != nil {
			t.Errorf("got %v, want no options for a command that declares none", options)
		}
	})
	...
	t.Run("Empty args with no options", func(t *testing.T) {
		...
		options := parseCommandOptions(cmd.Options, []string{})
		if len(options) > 0 {
	...
	t.Run("Excess args", func(t *testing.T) {
		...
		options := parseCommandOptions(cmd.Options, []string{"1", "extra", "args"})
		if len(options) != 1 {
