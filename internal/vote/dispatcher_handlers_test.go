...
func votingVote(t *testing.T, guildID string, requiredVotes int, onPassed func(*discordgo.Session, *Session, Tally)) *Session {
	...
	session := newVoteSession(guildID, KindSkip, "Skip", "⏭", "voice1", requiredVotes)
...
func TestReactionAddReachingQuorumRunsThePassPath(t *testing.T) {
	...
	var seen Tally
	vote := votingVote(t, "dispatch-pass", 2, func(_ *discordgo.Session, _ *Session, tally Tally) {
		passes.Add(1)
		seen = tally
	})
	...
	if _, live := activeVotes.snapshotOf("dispatch-pass", KindSkip); live {
...
func TestConcurrentReactionsPassTheVoteOnce(t *testing.T) {
	...
	var passes atomic.Int32
	vote := votingVote(t, "dispatch-race", 2, func(*discordgo.Session, *Session, Tally) { passes.Add(1) })
...
func TestReactionAddPassesWhenTheChannelEmpties(t *testing.T) {
	...
	var passes atomic.Int32
	var seen Tally
	vote := votingVote(t, "dispatch-shrink", 3, func(_ *discordgo.Session, _ *Session, tally Tally) {
		passes.Add(1)
		seen = tally
	})
...
func TestAbsentRequesterCanConsentWithoutMovingTheQuorum(t *testing.T) {
	...
	var passes atomic.Int32
	var seen Tally
	vote := votingVote(t, "dispatch-consent", 2, func(_ *discordgo.Session, _ *Session, tally Tally) {
		passes.Add(1)
		seen = tally
	})
...
func TestPresentRequesterCountsForBothPaths(t *testing.T) {
	...
	var seen Tally
	vote := votingVote(t, "dispatch-present-owner", 3, func(_ *discordgo.Session, _ *Session, tally Tally) {
		seen = tally
	})
