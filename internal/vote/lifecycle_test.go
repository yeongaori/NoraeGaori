...
func (v *voteStubs) useAdders(adders []string) {
	addersFor = func(string, Target) []string { return adders }
}
...
func liveVote(t *testing.T, guildID string, kind Kind) *Session {
	t.Helper()
...
func TestAwaitVoteOutcomeRendersACancellation(t *testing.T) {
	...
	session := liveVote(t, "lifecycle-cancel", KindSkip)
	...
	activeVotes.cancel("lifecycle-cancel", voteEndSuperseded, KindSkip)
...
func TestAwaitVoteOutcomeStaysQuietWhenTheVotePassed(t *testing.T) {
	stubs := stubVoteEffects(t)
	session := liveVote(t, "lifecycle-passed", KindSkip)
...
func TestAwaitVoteOutcomeExpires(t *testing.T) {
	...
	session := liveVote(t, "lifecycle-expiry", KindStop)
	...
	if _, live := activeVotes.snapshotOf("lifecycle-expiry", KindStop); live {
...
func TestAwaitVoteOutcomeSeedsItsOwnReaction(t *testing.T) {
	...
	session := liveVote(t, "lifecycle-seed", KindSkip)
...
func TestExpiredTimerDoesNotOverwriteAResolvedVote(t *testing.T) {
	...
	session := liveVote(t, "lifecycle-resolved", KindSkip)
...
func TestEndedBeforeAttachPrefersTheRecordedReason(t *testing.T) {
	session := newVoteSession("g1", KindSkip, "Vote", "⏭", "voice1", 2)
