...
func newTestRegistrySession(guildID string, kind Kind) *Session {
...
func TestClaimHoldsOneSessionPerKind(t *testing.T) {
	...
	first := newTestRegistrySession("g1", KindSkip)
...
func TestReleaseOnlyEvictsItsOwnSession(t *testing.T) {
	...
	session := newTestRegistrySession("g1", KindSkip)
	registry.claim(session)
	registry.attachMessage(session, "msg1", "chan1")

	registry.release(newTestRegistrySession("g1", KindSkip))
	if _, live := registry.snapshotOf("g1", KindSkip); !live {
...
func TestResolveSucceedsExactlyOnce(t *testing.T) {
	...
	session := newTestRegistrySession("g1", KindStop)
	registry.claim(session)
	registry.attachMessage(session, "msg1", "chan1")
...
func TestCancelEndsNamedKindsOnly(t *testing.T) {
	...
	skip := newTestRegistrySession("g1", KindSkip)
	stop := newTestRegistrySession("g1", KindStop)
	registry.claim(skip)
	registry.claim(stop)
...
func TestRetractVoteDecrements(t *testing.T) {
	...
	registry.recordVote(session, presentBallot("u2"), voteThreshold{quorum: 1})
	tally, withdrawn = registry.retractVote(session, voteBallot{userID: "u2"}, voteThreshold{quorum: 1, adders: []string{"u1"}})
	if !withdrawn || tally.current != 1 || tally.passed || tally.byAdderConsent {
...
func TestConcurrentClaimAndSnapshotStayConsistent(t *testing.T) {
	...
	for range 16 {
		...
		go func() {
			defer wg.Done()
			snapshot, live := registry.snapshotOf("g1", KindSkip)
